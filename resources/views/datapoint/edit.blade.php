...
@section('content')
  <section class="section">
    ...
    <div class="section-body">
      <h2 class="section-title">Edit Data Attributes</h2>
      ...
      <div class="col-12 col-md-6 col-lg-6">
        <div class="card">
          <form action="{{ route('datapoints.update', $datapoint->id) }}" method="post">
            @csrf
            @method('put')
            <div class="card-body">
              <div class="form-group">
                <label for="name">Name</label>
                <input type="text" class="form-control" id="name" name="name" value="{{ $datapoint->name }}" required>
              </div>
              ...
              <div class="form-group">
                ...
                <select class="form-control" id="type" name="type" required>
                  <option value="framework" {{ old('type', $datapoint->type) == 'framework' ? 'selected' : '' }}>Framework</option>
                  <option value="library" {{ old('type', $datapoint->type) == 'library' ? 'selected' : '' }}>Library</option>
                </select>
              </div>
              ...
              @foreach ($attributes as $attribute)
                <div class="form-group">
                  ...
                  <input type="number" class="form-control" id="attribute-{{ $attribute->id }}" name="attributes[{{ $attribute->id }}]"
                    value="{{ old('attributes.' . $attribute->id, $attribute->value) }}" required>
                </div>
              @endforeach
            </div>
